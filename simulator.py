...
def L2(v1, v2):
    dist = np.linalg.norm(np.array(v1) - v2)
    return dist
...
class RobotEnv():
    ...
    def get_joints(self):
        all_joints = p.getJointStates(self.robot_id, self.joints)
    ...
    def follow_path(self, path, loop_num=100):
        for p in path:
            self.move_to_joint(p, loop_num=loop_num)
    ...
    def move_to_joint(self, joints_values, joints_ids=None, max_force=60, loop_num=10):
        ...
        if self.use_force:
            ...
            #         self.robot_id, joints_ids,
            #         p.POSITION_CONTROL,
            #         targetPositions=joints_values,
            #         targetVelocities=[0.1]*self.joint_num,
            #         positionGains=[0.04]*self.joint_num, 
            #         forces=[max_force]*self.joint_num)
            ...
            # while True:
                ...
                # if np.linalg.norm(current_joints - joints_values) < 1e-2:
                #     break
        ...
        # while True:
        #     s = np.sum( np.abs(self.get_joints() - np.array(joints_values)) ) 
        #     if s < 1e-3:
        #         break
    ...
    def move_to(self, pos, quat, start_joints=None, rrt_range=0.1):
        ...
        if len(plan) > 0:
            ...
            self.move_to_joint(plan[-1])

        # for _ in range(100):
        #     pybullet.stepSimulation()

        if self.eye_on_hand:
            link_state = p.getLinkState(self.robot_id, 11)
    ...
    def check_collision(self, q):
        ...
        for obstacle_id in self.obstacles:
            closest_points = p.getClosestPoints(
                self.robot_id, obstacle_id, 0.18)
            if closest_points is not None and len(closest_points) != 0:
                return True
        return False


    def visualize_path(self, q_1, q_2, color=[0, 1, 0]):
        """
        ...
        self.set_joints(q_1)
        point_1 = p.getLinkState(self.robot_id, 7)[0]
        # obtain position of second point
        self.set_joints(q_2)
        point_2 = p.getLinkState(self.robot_id, 7)[0]
...
def add_box(size, pos, quat_xyzw=[0,0,0,1], color=[1,1,0,1]):
    box_id = p.createVisualShape(p.GEOM_BOX, halfExtents=size, rgbaColor=color)
